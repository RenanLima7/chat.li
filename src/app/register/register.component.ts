...
export class RegisterComponent {
  ...
  cadastro() {
    ...
    if (this.email === '' || this.name === '' || this.confirmarPassword === '' || this.genre === 'n-a' || this.password === '') {      
      this.errorForm = 'Preecha todos os campos';
      return;
    }

    if (this.password !== this.confirmarPassword) {
      this.errorForm = 'As senhas estão diferente';
      return;
    }

    if (this.password.length < 8) {
      this.errorForm = 'A senha deve ter no mínimo 8 caractéres';
      return;
    }
    ...
    // if (!this.avatar.startsWith('data:image/png')) {
    //   this.errorForm = 'Tipo de imagem inválido, suporte apenas para (PNG | JPG)';
    //   return;
    // }
    ...
    this.register.register({
      ...
      this.errorForm = 'Usuario ou senha invalidos. Tente novamente';
  });
